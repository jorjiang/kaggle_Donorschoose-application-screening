# -*- coding: utf-8 -*-
"""
Created on Tue Mar 27 05:03:26 2018
...
#def cal_rate(x): return 1 if x > 1 else x
#
#teacher['approved_rate'] = teacher.tac / teacher.sb
#teacher['approved_rate'] = list(map(cal_rate, teacher.approved_rate))
...
#plot heat map of sent data and output
# =============================================================================
# pol_rj_df = df_all[~df_all['project_is_approved'].isna()]
# 
# pol_rj_df = 1 - pol_rj_df.groupby(['essay_polarity','essay_subjectivity']).mean()['project_is_approved'].unstack()
# 
# 
# ax = sns.heatmap(pol_rj_df, cmap='viridis_r')
...
#improve idea
#1. cross two level of categories
...
# df_0 = df[df['project_is_approved'] == 0].sample(len(df_1), replace = True)
#  
#  
# df = pd.concat([df_0, df_1])
# del df_0, df_1
#  
# X_train = df.drop('project_is_approved', 1)
# y_train = df['project_is_approved']
#  
# del df 
# 
# clf = RandomForestClassifier().fit(X_train, y_train)
# pd.to_pickle(clf, 'RF_spl.pickle')
# acc = roc_auc_score(y_test, clf.predict(X_test))
# 
# submission = clf.predict(X_sub)
# submission = pd.Series(submission, index = X_sub.index).rename('project_is_approved').reset_index()
# submission.to_csv('RF_spl.csv', index = False)
# =============================================================================
